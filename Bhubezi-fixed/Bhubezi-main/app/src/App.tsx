...
const App: React.FC = () => {
  const STORAGE_KEY = 'bhubezi_auth';
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        const { user: storedUser, isOnboarded: storedOnboarded, role: storedRole } = JSON.parse(stored);
        if (storedOnboarded && storedUser) {
          setUser(storedUser);
  ...
  // Save state whenever user or isOnboarded changes
  useEffect(() => {
